...
Phase 1 における「ZWG が保持・検出できる会話ログ」の source of truth は
`%APPDATA%/zwg/vcc/logs/` 配下の `*.jsonl` 一覧とする。既存一覧がなければこの
ディレクトリ走査を唯一の参照元とし、新規のログ収集機構そのものはこのフェーズで
追加しない。
...
ソース選択そのものでは自動コンパイルしない。選択時は直近 manifest と既存成果物を
読み込み、成果物が無いか古い場合は `stale` を表示する。`Compile` はアクティブ
ソースに対する `full + min` の再生成を行う。`Search` は `View` 専用の生成要求と
して扱い、検索語確定時に発火する。前提成果物が不足していれば、同一リクエスト内で
必要な生成も合わせて行う。
...
`stale` / `ready` の判定はモード単位で行う。`Full` と `Min` はソース更新時刻に対して
判定し、`View` はそれに加えて `last_view_query` が現在の確定検索語と一致することを
条件に含める。

Phase 1 の `View` は「最後に確定した 1 クエリだけを保持する」方式とする。同一
ソース・同一出力先ごとに `.view.txt` は 1 つだけ持ち、新しい検索語で生成した時点で
上書きする。
...
- 生成対象モード
- 検索語
- `truncate` — brief mode の標準トークン上限。Phase 1 は `128` で固定
- `truncate_user` — brief mode の user ブロック用トークン上限。Phase 1 は `256` で固定

`truncate` / `truncate_user` は VCC 既定値互換の内部パラメータとして扱い、Phase 1
では UI・設定ファイルから変更できない。
...
`view_generated_at` と `last_view_query` は `.view.txt` の再利用可否判定に使う。
...
Phase 1 では `mojo run` を実行環境要件として許容する。つまり `VCC Viewer` を使う
環境には Mojo CLI がインストールされている必要がある。配布物への同梱やネイティブ
バイナリ化は後続フェーズで扱う。
