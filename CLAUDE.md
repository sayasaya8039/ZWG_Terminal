...
| **言語選択** | CLIツール→Zig、API/サービス→Go、GUI→Rust+gpui、デスクトップ(TS)→Electrobun、Web→TypeScript/Svelte、高速Python代替→Mojo |
...
| **Mojo** | 0.26+ | Python互換の高速言語（WSL経由、`~/mojo-env`） |
...
| 項目 | 値 |
|------|-----|
| **バージョン** | Mojo 0.26.3 (nightly) |
| **プラットフォーム** | WSL2 (Linux x86_64) — Windowsネイティブ未対応 |
| **仮想環境** | `~/mojo-env` |
| **実行方法** | `wsl -e bash -c "source ~/mojo-env/bin/activate && mojo run file.mojo"` |
| **フォーマッター** | `mblack`（同梱） |
...
| 用途 | 説明 |
|------|------|
| Python高速化 | Pythonコードの68,000倍高速化が可能 |
| SIMD/並列処理 | ハードウェアレベルの最適化 |
| AI/MLワークロード | Python互換でGPU対応 |

#### 注意事項

- `fn` は非推奨、`def` を使用すること
- テレメトリ無効化: `MODULAR_TELEMETRY_ENABLED=0`
- アップデート: `wsl -e bash -c "source ~/mojo-env/bin/activate && uv pip install --upgrade mojo --index https://whl.modular.com/nightly/simple/ --prerelease allow"`
